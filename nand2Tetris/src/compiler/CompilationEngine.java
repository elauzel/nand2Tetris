...
/**
 * Recursive top-down parser that effects the actual compilation output. Gets its input from a JackTokenizer and emits its parsed structure into an output
 * file/stream. The output is generated by a series of compilexxx() routines, one for every syntactic element xxx of the Jack grammar. The contract
 * between these routines is that each compilexxx() routine should read the syntactic construct xxx from the input, advance() the tokenizer exactly beyond
 * xxx, and output the parsing of xxx. Thus, compilexxx() may only be called if indeed xxx is the next syntactic element of the input.
 * 
...
public class CompilationEngine {
	...
	private void checkForSubroutineDecs() {
		while (true) {
			...
			if (tokenizer.keyWord() == KeyWord.CONSTRUCTOR || tokenizer.keyWord() == KeyWord.FUNCTION
	...
	private void writeSubroutineVM(KeyWord subroutineType, String subroutineName, String varType) {
		...
		if (subroutineType == KeyWord.CONSTRUCTOR) {
			...
			vmw.writePush(Segment.CONST, nFields);
			vmw.writeCall("Memory.alloc", 1);
			vmw.writePop(Segment.POINTER, 0);
		}

		// set 'this' to point to the object passed as the first Argument in a method
		if (subroutineType == KeyWord.METHOD) {
			TABLE.define("~this~", "this", Identifier.ARG);
	...
	private void compileIf() {
		...
		if (tokenizer.tokenType() == TokenType.KEYWORD && tokenizer.keyWord() == KeyWord.ELSE) {
			...
			String endLabel = uniqueLabel("IF_END");
	 ...
	 * if the current token is an identifier, the routine must distinguish between a variable, an array entry, and a subroutine call. A single look-ahead
	 * token, which may be one of "[", "(", or "." suffices to distinguish between the three possibilities. Any other token is not part of this term and
	...
	private void writeTermIdentifiers() {
		...
		if (tokenizer.tokenType() != TokenType.SYMBOL) throwException("There should be a symbol here!");

		// '[' expression ']' | '.' subroutineName '(' expressionList ')' | '(' expressionList ')'
		switch (tokenizer.symbol()) {
			...
			case '(': // '(' expressionList ')'
				...
				break;
			default:
				// push argument onto the stack since it's a variable name
				int varIndex = TABLE.indexOf(name);
				Identifier varKind = TABLE.kindOf(name);
				Segment varSeg = TABLE.kindToSegment(varKind);
				vmw.writePush(varSeg, varIndex, varKind, name);
		}
